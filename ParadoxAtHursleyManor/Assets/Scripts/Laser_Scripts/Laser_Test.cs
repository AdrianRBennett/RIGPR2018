...
public class Laser_Test : MonoBehaviour {
    ...
    {    
        ...
        if (Physics.Raycast(origin, direction, out hit, 1000.0f))    // Here the raycast is created and it detects when it hits something.
        {
            ...
            laserArray[counter].transform.LookAt(hit.point);     // Next it rotates the object to look where the ray point has hit (LookAt() function is a little tricky but found a work around).
        ...
        {
            Debug.DrawLine(origin, (origin + direction), Color.blue, 0.1f);
